...
def risk_components(hazard: HazardField) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "critical_infra": infra_proximity(),
    }


def composite_risk(hazard: HazardField) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    components = risk_components(hazard)
